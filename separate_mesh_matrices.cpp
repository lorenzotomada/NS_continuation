...
// ------------------------------------ NS CLASS -----------------------------------
  ...
  class NS
  {
    ...
    void assemble_system(const bool steady_system=false);
    void assemble_rhs(const bool steady_system=false);


    void solve(const bool steady_system=false);
    ...
    PETScWrappers::MPI::BlockSparseMatrix M_matrix;
    PETScWrappers::MPI::BlockSparseMatrix A_matrix;
    PETScWrappers::MPI::BlockSparseMatrix B_matrix;
...
// --------------------- SETUP SYSTEM ---------------------
  ...
  void NS<dim>::setup_system()
  {
    ...
    A_matrix.reinit(owned_partitioning, dsp, mpi_communicator);
    B_matrix.reinit(owned_partitioning, dsp, mpi_communicator);
    M_matrix.reinit(owned_partitioning, dsp, mpi_communicator);
...
// -------------------- ASSEMBLE THE JACOBIAN --------------------
  ...
  template <int dim>
  void NS<dim>::assemble(const bool assemble_jacobian, const bool steady_system)
  {
    ...
    FullMatrix<double> local_A_matrix(dofs_per_cell, dofs_per_cell);
    FullMatrix<double> local_B_matrix(dofs_per_cell, dofs_per_cell);
    FullMatrix<double> local_M_matrix(dofs_per_cell, dofs_per_cell);
    ...
    for (const auto &cell : dof_handler.active_cell_iterators())
    {
      if (cell->is_locally_owned())
      {
        ...
        for (unsigned int q = 0; q < n_q_points; ++q)
        {
          ...
          for (unsigned int i = 0; i < dofs_per_cell; ++i)
          {
            if (assemble_jacobian)
            {
              ...
              {
                double quantity_A; // A
                double quantity_B; // B, B^T
                double quantity_M;

                if (build_mesh_matrices)                  // if we do not have to build the mesh matrices, then there is no
                                                          // need to compute these quantities, as they only depend on the mesh
                {
                  quantity_A = scalar_product(grad_phi_u[i], grad_phi_u[j]); //  1/mu*a(u, v) ---> grad(u)*grad(v) 
                  quantity_B = - div_phi_u[i] * phi_p[j];
                  quantity_M = (phi_u[i] * phi_u[j]) / time.get_delta_t();           //  1/dt*M, M <--- u*u, coming from time stepping


                  if (steady_system)
                  {
                    local_B_matrix(i, j) += ( // add the stabilization term
                        gamma * div_phi_u[i] * div_phi_u[j]
                        + phi_p[i] * phi_p[j]
                        ) * fe_values.JxW(q);
                    quantity_B += - phi_p[i] * div_phi_u[j];                          //  b(u, q) = -q*div(u)
                  }
                  ...
                  local_M_matrix(i, j) += quantity_M * fe_values.JxW(q);
                  local_A_matrix(i, j) += quantity_A * fe_values.JxW(q);
                  local_B_matrix(i, j) += quantity_B * fe_values.JxW(q);
                }

                // the matrices depending on the mesh have been built

                const double quantity_C = // build C
                ...
                if (!steady_system)
                  local_matrix(i, j) += (theta * quantity_C) * fe_values.JxW(q); // the other terms can be added later on
                

                else // if steady system:
                  local_matrix(i, j) += quantity_C * fe_values.JxW(q); // the other terms can be added later on
              }
            }
            ...
            const double quantity_C = - phi_u[i] * (present_velocity_gradients[q] * present_velocity_values[q]); //  from C

            // not at the first iteration: we can just assemble mesh matrices (if needed) and use them

            if (steady_system)
              local_rhs(i) += (quantity_C - gamma * div_phi_u[i] * present_velocity_divergence) * fe_values.JxW(q);

            else
            {
              local_rhs(i) += theta * quantity_C * fe_values.JxW(q);
              local_rhs(i) += ( (1-theta)*
                        (
        ...
        if (assemble_jacobian)
        { 
          empty_constraints.distribute_local_to_global(local_matrix, local_dof_indices, jacobian_matrix); // here we do not take into account
                                                                                                           // boundary conditions
          if (build_mesh_matrices)
          {
            empty_constraints.distribute_local_to_global(local_M_matrix, local_dof_indices, M_matrix);
            empty_constraints.distribute_local_to_global(local_A_matrix, local_dof_indices, A_matrix);
            empty_constraints.distribute_local_to_global(local_B_matrix, local_dof_indices, B_matrix);
          }
        }
        
        empty_constraints.distribute_local_to_global(local_rhs, local_dof_indices, system_rhs);
      }
    }
    ...
    if (assemble_jacobian)
    {
      ...
      if (build_mesh_matrices)
      { 
        M_matrix.compress(VectorOperation::add);
        A_matrix.compress(VectorOperation::add);
        B_matrix.compress(VectorOperation::add);
        pcout << "  The matrices depending only on the mesh have been assembled" << std::endl;
      }
 

      if (!steady_system)
      {
        jacobian_matrix.add(theta * viscosity, A_matrix);
        jacobian_matrix.add(1.0, B_matrix);
        jacobian_matrix.add(1.0, M_matrix);
      }

      else
      {
        jacobian_matrix.add(viscosity, A_matrix);
        jacobian_matrix.add(1.0, B_matrix);
      }
    ...
    // now build the rhs
      ...
      PETScWrappers::MPI::BlockVector tmp{evaluation_points};
      tmp = 0;

      if (steady_system)
      {
        tmp.add(-viscosity, evaluation_points);
        A_matrix.vmult_add(system_rhs, tmp);

        tmp = 0;
        tmp.add(-1.0, evaluation_points);
        B_matrix.vmult_add(system_rhs, tmp);
      }
      else
      {
        tmp.add(-theta*viscosity, evaluation_points);
        A_matrix.vmult_add(system_rhs, tmp);

        tmp = 0;
        tmp.add(theta*viscosity, old_solution);
        A_matrix.vmult_add(system_rhs, tmp);

        tmp = old_solution;
        tmp.add(-1.0, evaluation_points);
        M_matrix.vmult_add(system_rhs, tmp);
...
// -------------------- ASSEMBLE THE SYSTEM --------------------
  ...
  template <int dim>
  void NS<dim>::assemble_system(const bool steady_system)
  {
    const bool assemble_jacobian{true}; // if this function is called, we need to assemble the Jacobian matrix


    assemble(assemble_jacobian, steady_system); // just call the assemble function
...
// -------------------- ASSEMBLE THE RESIDUAL --------------------
  ...
  template <int dim>
  void NS<dim>::assemble_rhs(const bool steady_system)
  {
    const bool assemble_jacobian{false};
    assemble(assemble_jacobian, steady_system);
  }
...
// -------------------- SOLVE --------------------
  ...
  template <int dim>
  void NS<dim>::solve(const bool steady_system)
  {
                                ...
                                )
  {
    ...
    while ((first_iteration || (current_res > tolerance)) && line_search_n < max_n_line_searches)
    {
      if (first_iteration)
      {
        ...
        assemble_rhs(steady_system);

        //nonzero_constraints.distribute(system_rhs);
      ...
      else
      {
        ...
        for (double alpha = 1.0; alpha > 1e-5; alpha *= 0.5)
        {
          ...
          assemble_rhs(steady_system); // we assemble taking into account the boundary conditions
...
// -------------------- COMPUTE INITIAL GUESS --------------------
  ...
  void NS<dim>::compute_initial_guess()
  {
    ...
    {
      const double fraction_to_refine{0.7};
